    ...
    <div>
      <Table className={styles.Body}>
        <thead>
          <tr>
            ...
            <th>{isSmallScreen ? "AM" : "Asesinato más distante"}</th>
            <th>{isSmallScreen ? "T" : "Tiempo de juego"}</th>
            <th>{isSmallScreen ? "S" : "Suicidios"}</th>
          </tr>
        </thead>
        <tbody>
          {data?.map((stats) => (
            <tr key={stats.id} onClick={() => handleRowClick(stats)}>
              <td>{stats.rank}</td>
              <td>{stats.name}</td>
              <td>
                <svg
      ...
      {selectedPlayer && (
        <div className={styles.PlayerOverviewCard}>
          <button onClick={handleCloseOverview} className={styles.close}>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="16"
